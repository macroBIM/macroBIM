...
**크레딧은 목록의 `라이선스 유형` 열 하나로 정해집니다.** 유투브 오디오
보관함에서 이 곡 줄의 아이콘을 봅니다 — 유투브 로고면 필요 없고, CC(사람
모양)면 설명란에 필수입니다. 골든게이트에 쓴 `Survival Mode` 는 유투브
로고였습니다. 필요할 때만:
...
어느 쪽이든 **Content ID 클레임은 붙지 않습니다** — 보관함 곡이니까.
...
- 공개 여부
- 이 곡의 `라이선스 유형` 아이콘 확인 — 유투브 로고면 설명란 크레딧 줄을 지운다 (§6-1)
