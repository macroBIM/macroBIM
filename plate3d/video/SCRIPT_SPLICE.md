...
| | 셀 | 값 | 같이 변하는 것 |
|---|---|---|---|
| ① 단면 | `PARAM!C6` `C7` | H-300×300 → **H-400×400** | D..I 여섯 칸(H·B·t1·t2·r·kg/m)이 드롭다운 하나로 채워짐 · clear web depth · member centre |
| ② 볼트 | `PARAM!C28` `C30` | Long N 4 → **6** | flange pitch · 볼트 총수 |
| ③ 간격 | `PARAM!C8` | 20 → **40** | 두 부재 사이가 벌어짐 |

**①이 이 영상의 핵심입니다.** 드롭다운에서 규격을 고르면 여섯 칸이 스스로 채워집니다 —
KS 58개 규격 표가 시트 안에 들어 있고, 그걸 참조하는 수식이 이미 걸려 있기 때문입니다.
사람이 치는 건 이름 하나입니다.

**단, 판은 따라오지 않습니다.** 판 치수는 파란 입력이지 수식이 아니라서, 400 플랜지에
280 판이 그대로 남습니다. 영상에서 이걸 숨기면 거짓말이 되므로, ① 다음에 **상부 판
폭을 280 → 380 으로 고치는 컷을 반 초 붙입니다.** "고쳐야 할 것은 시트가 알려 준다"가
오히려 이 도구의 성격에 맞습니다.
