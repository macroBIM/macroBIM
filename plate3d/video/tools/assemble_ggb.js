/* Turn the captured stills into the Golden Gate film.
   ...
   The same three passes the other assemblers use, pointed at shots_ggb.json:
...
const SP = __dirname;
const meta = JSON.parse(fs.readFileSync(path.join(SP, 'shots_ggb.json'), 'utf8'));
const SRC = path.join(SP, meta.src);
const NORM = path.join(SP, 'ggb_norm'), SEQ = path.join(SP, 'ggb_seq');
...
fs.writeFileSync(path.join(SP, 'filter_ggb.txt'), parts.join(';'));
...
cp.execFileSync(FF, ['-hide_banner', '-loglevel', 'error', '-y',
  ...inputs,
  '-filter_complex_script', path.join(SP, 'filter_ggb.txt'), '-map', '[vout]',
  '-t', DUR.toFixed(3),
