/* Turn the captured stills into the tower-crane film. Same three passes as the
   first film's assembler, pointed at the other shots.json.

   Turn the captured stills into the film.

   Three passes, because each is cheap and none of them is clever:

   1. Normalise. Frames arrive at three different sizes - the WebGL canvas, the
      1920x1080 flat pages, and the full browser window for the two shots that
      show the app chrome. An image sequence has to be one size, so everything
      is scaled to fill 1920x1080 and centre-cropped.
...
const SP = __dirname;
const SRC = SP + '/src', NORM = SP + '/norm', SEQ = SP + '/seq';
const W = 1920, H = 1080;
const meta = JSON.parse(fs.readFileSync(SP + '/shots.json', 'utf8'));
const FPS = meta.fps;
...
meta.caps.forEach((c, i) => {
  inputs.push('-loop', '1', '-t', String(c.dur + 1), '-i', SP + '/' + c.png);
  const FD = 0.35;
