/* Turn the captured stills into the Simple connector film. Same three passes as the tower
...
const SP = __dirname;
const meta = JSON.parse(fs.readFileSync(SP + '/shots_simpleconn.json', 'utf8'));
const SRC = SP + '/' + (meta.dir || 'src_simpleconn');
const NORM = SP + '/norm_simpleconn', SEQ = SP + '/seq_simpleconn';
...
meta.shots.forEach(s => {
  ...
  const src = '../norm_simpleconn/' + s.file;
...
fs.writeFileSync(SP + '/filter_simpleconn.txt', parts.join(';'));

const OUT = SP + '/../PLATE3D_SIMPLECONN.mp4';
console.log('encoding ' + meta.caps.length + ' captions over ' + DUR.toFixed(1) + ' s ...');
cp.execFileSync(FF, ['-hide_banner', '-loglevel', 'error', '-y',
  ...inputs,
  '-filter_complex_script', SP + '/filter_simpleconn.txt', '-map', '[vout]',
  '-t', DUR.toFixed(3),
