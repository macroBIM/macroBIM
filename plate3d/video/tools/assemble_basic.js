/* Turn the captured stills into the teaching film. Same three passes as the
...
const SRC = SP + '/' + (meta.dir || 'src_basic');
const NORM = SP + '/norm_basic', SEQ = SP + '/seq_basic';
...
fs.writeFileSync(SP + '/filter_basic.txt', parts.join(';'));

const OUT = SP + '/../PLATE3D_BASIC.mp4';
console.log('encoding ' + meta.caps.length + ' captions over ' + DUR.toFixed(1) + ' s ...');
cp.execFileSync(FF, ['-hide_banner', '-loglevel', 'error', '-y',
  ...inputs,
  '-filter_complex_script', SP + '/filter_basic.txt', '-map', '[vout]',
  '-t', DUR.toFixed(3),
