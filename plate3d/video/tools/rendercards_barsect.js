/* The teaching film's cards, screenshotted with an alpha channel so ffmpeg can
   ...
   Which cards keep their background is the whole of the logic here:
     ...
     pills   s3_c*   transparent - a lozenge over live footage
     section s3_s*   transparent - and this is the subtle one. The scrim is the
     ...
     title   s3_t02  solid        - it replaces the picture outright
     outro   s3_o31  solid        - so does it                                 */
const { chromium } = require('playwright-core');
const fs = require('fs');
const SP = __dirname + '/barsect';
(async () => {
  const b = await chromium.launch({ executablePath: '/opt/pw-browsers/chromium', args: ['--no-sandbox'] });
  const p = await b.newPage({ viewport: { width: 1920, height: 1080 } });
  await p.route('**/fonts.{googleapis,gstatic}.com/**', r => r.abort());
  const files = fs.readdirSync(SP).filter(f => /^s3_.*\.html$/.test(f)).sort();
