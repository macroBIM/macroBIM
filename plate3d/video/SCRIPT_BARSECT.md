...
**개념부터 말합니다.** 판은 **모양이 곧 부재**입니다 — 외곽선을 그리면 그게 판입니다.
봉과 형강은 **단면과 길이가 따로**입니다:

```
    단면                      길이
    카탈로그에서 고른다        어디에 놓느냐가 정한다
    H-200x200x8x12            2,400  ← 두 점 사이 거리
    Ø24                       또는 SECT 행에 적은 값
```

그래서 앱도 왼쪽 목록을 **PLATES · BARS · SECTIONS 세 표로 나눠** 둡니다. 만들어지는
방식이 다르기 때문입니다. **SECTIONS 표를 클릭하면 단면 도면이 열리고 치수를 잴 수 있습니다** —
판에는 없는 것입니다.
...
| 규격 | 3:53 · 2560×1440 · 30 fps · **무음 · 영어 자막만** |
...
0:24  BAR — one row
0:38  SECT — H, C and L
1:17  Two ways to place it
1:56  Work points, and the steel
2:42  Repeating, and placing
```
...
| **4** |  | 0:16 | 8s | PLATE 행과 SECT 행을 나란히 — 외곽선 vs 카탈로그 치수 | **A plate you draw.**<br>**A profile you choose — then give it a length.** |
| 5 |  | 0:24 | 7s | 시트 BAR 행 + 왼쪽 **BARS** 표 | **BAR is one row.**<br>**Material, diameter, length.** |
| 6 |  | 0:31 | 7s | `preview BAR.ANCH` — 원형 단면 | *A round bar. There is no shape to draw.* |
| **7** |  | 0:38 | 8s | 시트 SECT 4행, TYPE 열에 링 | **SECT is a rolled profile.**<br>**H, C or L.** |
| **8** |  | 0:46 | 8s | **SECTIONS** 표 클릭 → 단면 도면 열림 · measure | **The numbers are the catalogue’s.**<br>**The fillets are drawn as real arcs.** |
...
| **12** | S | 1:17 | 5s | **섹션 카드** — 브레이스가 남은 채 | **A profile needs a length.**<br>**Where does it come from?** |
| **13** |  | 1:22 | 9s | `md.col` 의 **두 블록이 나란히** — 37~40행과 42~45행, 8번째 칸에만 링 | **One keyword, two grammars.**<br>**The eighth column decides.** |
| **14** |  | 1:31 | 9s | **평면 방식** — `sc.col … 0 0 25 XY` · 기둥이 서고 길이는 SECT 행에서 | **A plane name: stand it here, run it the plane’s way.**<br>**XY is +Z. XZ is −Y. YZ is +X.** |
| **15** |  | 1:40 | 9s | **좌표 방식** — `sc.brc 0 -150 0 → 2400 -150 3000` · 브레이스가 늘어남 | **Two points instead.**<br>**Stretch it from here, to there.** |
| 16 |  | 1:49 | 7s | SECTIONS 표 길이 칸의 **`ref`** 표시 | **Then the Length in the SECT row is only a reference.**<br>**The app says so.** |
| **17** | S | 1:56 | 5s | **섹션 카드** — 브레이스 끝단 위로 | **The two points are work lines.**<br>**The steel is not.** |
| **18** |  | 2:01 | 9s | 브레이스 끝 **확대** · 절점과 강재 끝 사이 170 | **Type the node. Then trim the steel.** |
| **19** |  | 2:10 | 9s | `OFF_B 0` ↔ `170` — 강재가 절점에서 물러남 | **Positive pulls back from the node.** |
| **20** |  | 2:19 | 9s | 앵커볼트 `-50` · 베이스판을 **뚫고 들어감** | **Negative runs past it —**<br>**into a gusset, into a base plate.** |
| 21 |  | 2:28 | 7s | 실제 길이 = 두 점 거리 − 양단 OFF | *Real length is the distance, less both ends.* |
| 22 |  | 2:35 | 7s | `sc.str` 의 `Alpha 90` ↔ `0` · 카메라 고정 | **Alpha turns it about its own axis.**<br>**The two points do not move.** |
| **23** | S | 2:42 | 5s | **섹션 카드** — 앵커 한 개가 남은 채 | **One row is one member.**<br>**How do you get a hundred?** |
| **24** |  | 2:47 | 9s | 좌표 형식 뒤 **반복 8칸** — 앵커 4행이 1행으로 | **dx dy dz repeat, twice over.**<br>**On both grammars.** |
| **25** |  | 2:56 | 8s | `ASSY as.bent md.col ADD 0 0 0` | **ASSY places what a MODULE built.** |
| 26 |  | 3:04 | 7s | `MIR 1200 0 0 YZ` — 기둥이 둘 | *MIR — mirrored about a plane you name.* |
| 27 |  | 3:11 | 7s | `COPY 0 3000 0 2` — 벤트가 셋 | *COPY — pushed along, repeat times.* |
| 28 |  | 3:18 | 7s | `ROT 0 0 25 Z 90 3` — 스티프너가 넷 | *ROT — swung round an axis, repeat times.* |
| **29** |  | 3:25 | 8s | **MODULE 없이** BAR·SECT 를 ASSY 에 바로 — `modules 0` | **A bar can skip the module**<br>**and go straight into the assembly.** |
| 30 |  | 3:33 | 7s | SECT 에 CUT → **길이 전체가 잘림** | **A CUT on a section cuts all of it.**<br>**Not a hole — a saw.** |
| **31** |  | 3:40 | 8s | 완성 벤트가 천천히 돔 | **PLATE. BAR. SECT.**<br>**One sheet.** |
| **32** | T | 3:48 | 5s | 로고 | **PLATE3D by macroBIM** |
...
| `C22` | 22 | `sc.str` 의 `Alpha` 를 `90` → `0` | 부재 수·**중량 동일** (돌기만 함) |
| `C30` | 30 | `sc.str` 에 `CUT` 행 추가 | **중량 줄어듦** — 길이 전체가 잘린 증거 |
| `B22` | 24 | 01편 것 재사용 | 이미 검증됨 |

**`C22` 의 중량이 소수점까지 같아야 합니다.** Alpha 는 부재를 돌릴 뿐 옮기지도 자르지도
