...
- **왼쪽 ASSEMBLY 목록은 ASSY 하나에 그룹 하나.** MIR/COPY/ROT로 파생된 조립체는 새 ID를 받지만
  같은 조립체를 옮긴 것이므로 **원본 그룹 안에 한 줄씩** 접혀 들어간다 (`AS.COMB.CP.001 · 1 module + 1 part`).
  그룹 체크박스로 파생본까지 한 번에 껐다 켤 수 있고, 각 줄은 자기 사본만 껐다 켠다.
  다른 ASSY를 `ADD`로 불러다 쓴 경우(`as.big`)는 사용자가 직접 이름 붙인 별개 조립체라 독립 그룹으로 남는다.
  판 단위는 MODULE 목록과 미리보기에서 확인.
