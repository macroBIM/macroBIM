...
| ⚠ | **모듈 안에서 부재 ID 중복 금지** | 부재 ID 는 BASE 행·부재 숨김·구성 목록이 모두 그 이름으로 지목하는 주소다. 한 모듈에서 같은 ID 가 두 번 나오면 **중대 오류로 시트 전체를 거부**한다 — 메시지 창이 두 행 번호를 알려 주고 **아무것도 그리지 않는다**. 같은 단면/판을 여러 번 쓰려면 `ID_1`, `ID_2` … 로 접미사를 붙인다(엔진이 `_숫자`를 떼고 정의를 찾는다) |
