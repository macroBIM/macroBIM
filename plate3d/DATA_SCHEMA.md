...
| CUT | [판ID], [기준점], RECT, B, H, L.X, L.Y, L.ROT, dx, dy, repeat | 사각 구멍/노치 |
...
- CUT 위치: CIRC는 중심, RECT/PLATE는 좌하단 기준
...
- **measure 체크박스**: 3D 화면 상단과 모듈 미리보기 헤더에 있음. 켜면 판 모서리·구멍 중심에
  스냅되고, 두 점을 클릭하면 ΔX·ΔY·ΔZ와 직선거리가 표시된다 (세 번째 클릭 = 새로 측정).
  측정 중에도 드래그 회전은 그대로 동작한다.
