...
| TYPE | 값 | 뜻 |
|---|---|---|
| **H** | `h bb bt tw tf1 tf2 r1 r2` | h=전체 높이, bb=하부 플랜지 폭, bt=상부 플랜지 폭, tw=웹 두께, **tf1=하부** 플랜지 두께, **tf2=상부** 플랜지 두께, r1=루트 필렛, r2=플랜지 토 라운딩 |
| **C** | `h b tw tf rw rf` | h=전체 높이, b=플랜지 폭, tw=웹 두께, tf=플랜지 두께, **rw**=웹/플랜지 루트 필렛, **rf**=플랜지 토 라운딩 |
| **L** | `a b t1 t2 r1 r2` | a=가로 다리, b=세로 다리, **t1=a 다리 두께**, **t2=b 다리 두께**, r1=루트 필렛, r2=토 라운딩 |

- C와 L은 값 개수가 같지만 **의미가 다르므로 타입별로 읽는다.** 등변 앵글도 `a b t1 t2`를 모두 적는다 (예: `100 100 10 10`).
...
- **SECT 입력값 검증** — 아래를 어기면 그 행은 **경고만 출력하고 만들지 않는다** (억지로 고치지 않음).
  ...
  반지름 음수 / TYPE이 H·C·L이 아님 / BASE.pt 이름 오류.
