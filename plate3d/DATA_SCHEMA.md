...
- **왼쪽 ASSEMBLY 목록은 ASSY ID 하나에 그룹 하나**이고, 그 안에 **구성원 ID 한 줄씩** 들어간다
  ...
  있고, 각 줄은 자기 구성원만 껐다 켠다. 줄마다 그 안에 든 모듈(또는 낱장 판)의 색이 붙고,
  줄을 클릭하면 그 모듈 미리보기가 열린다. 다른 ASSY를 통째로 넣은 줄은 여러 모듈이 섞여 있으므로
  색 없이 개수(`1 module + 1 part`)만 표시된다. 판 단위는 MODULE 목록과 미리보기에서 확인.
- **그룹 식별 색 띠 + 접기** — 조립체마다 왼쪽에 고유 색 띠가 헤더부터 구성원 줄까지 이어진다.
  **목록에서 그룹을 구분하기 위한 표식일 뿐 3D 색과는 무관**하다 (3D는 모듈/판 색 그대로).
  **ASSY ID 맨 앞의 원형 ⌄ 아이콘**(글자 크기, 그 조립체 색)을 누르면 구성원 줄이 접히고
  아이콘이 90° 돌아간다. 헤더에는 구성원 수가 같이 표시되어 접어 둔 채로도 규모를 알 수 있고,
  접힘 상태는 ID 기준으로 기억하므로 색을 바꿔 목록이 다시 그려져도 유지된다.
