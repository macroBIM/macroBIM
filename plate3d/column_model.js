...
(function (root, factory) {
  ...
  function defaults(env, cat) {
    ...
    const V = {
      ...
      /* No scallop, and no corner clip. A plate cut to the full clear space
         ...
         every side, so it touches neither flange nor web. Measured on
         H-300x300x10x15 r18 with the clip off: 270 deep clashes, 236 does not;
  ...
  function build(V, cat) {
    ...
    /* The stiffeners ride INSIDE md.c2 rather than in a module of their own, and
       ...
       TWO rows a level, one each side of the web. They used to differ - the
       far one carried ROT.Z 180 so its clipped corners would face the web as
       well - but with no clip the plate is a plain rectangle about its own mc,
       so the two are the same piece put in two places. One PLATE definition
       either way, so the take-off shows one item a level. */
    note2('');
    ...
    note2('Two rows a level. The web splits the space between the flanges in two, and one plate goes in each half.');
    ...
    V.stf.forEach((s, i) => {
      ...
      row(['MODULE', 'md.c2', on, 'mc', 0, f(stfHalf, stfHalfV), z, 'XY', 0, 0, 0],
          i === 0 ? 'and the far side of the web' : '');
    });
