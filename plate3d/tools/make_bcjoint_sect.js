/* The same beam-to-column connection, written with sections instead of plates.

   The plate version exists because a CUT cannot drill a section. This one
   gives that up and asks what is left: the members are one row each, the
   take-off names them the way a fabricator does, the fillets are real, and the
   bolts are still there to look at. Only the holes are missing.
   ...
   read off the model, not remembered.
     ...
     node tools/make_bcjoint_sect.js                                         */
...
const OUT = P3 + '/PLATE3D_BCJOINT_S.xlsx';
...
(async () => {
  ...
  console.log('  PLATE3D_BCJOINT_S.xlsx  ' + rows.length + ' rows');
