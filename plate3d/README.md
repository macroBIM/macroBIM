...
- **`plate_builder.js`** — 뷰어 엔진 (DATA_SCHEMA.md 구현). HTML에는 링크만 걸면 됨
  ...
  - **SECT(형강 H/C/L)** — 세 타입이 같은 입력 칸을 쓰고 필요 없는 값은 짝을 따라감(H 대칭, L 등변).
    필렛(r)과 토 라운딩(r2)을 실제 원호로 작도 — H-400×200×8×13 기준 면적 오차 0.06%. 배치는 BAR와 동일
  ...
  - **measure**: 3D 화면·모듈 미리보기·판 2D 도면에서 두 점을 클릭해 거리·ΔX/ΔY/ΔZ 측정.
    결과는 창 하단 줄에 남고(커서가 나가도 유지), 스냅 대상은 9점 + 구멍 중심 + **절단된 외곽선의 모든 꼭짓점**(노치 모서리·구멍 가장자리)
