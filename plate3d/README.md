...
- **`plate_builder.js`** — 뷰어 엔진 (DATA_SCHEMA.md 구현). HTML에는 링크만 걸면 됨
  ...
  - **그룹마다 왼쪽에 고유 색 띠** — 헤더부터 구성원 줄까지 이어져 어디서 한 조립체가 끝나고
    다음이 시작하는지 바로 보인다. **목록 표식일 뿐이라 3D에는 영향 없음**(3D 색은 모듈/판 색).
  - **접기 아이콘은 ASSY ID 맨 앞의 원형 ⌄ 아이콘** — 글자 크기(15px)로 그리고 그 조립체의
    색을 입혔다(= 그룹 표식 겸용). 누르면 구성원 줄이 접히고 아이콘이 90° 돌아 ›  모양이 된다.
    헤더에 구성원 수가 함께 표시되고, 접힘 상태는 색 변경 등으로 목록이 다시 그려져도 유지된다
