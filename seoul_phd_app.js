/*
    ...
    · 단면: box1cell / box12cell / ibeam / rect / circle / octagon / track (2D front only)
    · Toggle Normals: 벽면 중앙에 콘크리트 안쪽 법선 벡터 표시
*/
(function () {
  var BRANCH = 'claude/bim-box12cell-variables-k9f9oo';
  ...
  function loadBranchModules(done) {
    (function next(i) {
      ...
      fetch(RAW + BRANCH_MODULES[i] + '?v=' + Date.now())
        .then(function (r) { if (!r.ok) throw new Error(BRANCH_MODULES[i] + ' HTTP ' + r.status); return r.text(); })
        .then(function (code) { var s = document.createElement('script'); s.textContent = code; document.head.appendChild(s); })
        .catch(function (e) { console.error('[SeoulPhD] 브랜치 모듈 로드 실패:', e); })
        .then(function () { next(i + 1); });
    })(0);
  }
  (function load(i) {
    if (i >= ENGINE.length) { loadBranchModules(start); return; }
  ...
  function start() {
    ...
    ['box1cell', 'box12cell', 'ibeam', 'rect', 'circle', 'octagon', 'track'].forEach(function (k) {
      window['render_' + k + '_3d'] = function () {};
    });

    /* ─ SeoulPhD (드롭다운 디스패처 + 법선) ─ */
    var SeoulPhD = {
      sections: ['box1cell', 'box12cell', 'ibeam', 'rect', 'circle', 'octagon', 'track'],
      domPfx: { box1cell: 'box1cell', box12cell: 'box12cell', ibeam: 'ibeam', rect: 'rect', circle: 'circle', octagon: 'oct', track: 'track' },
