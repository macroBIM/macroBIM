/* beam_multi.js — MacroBEAM · MultiBEAM (PRODUCTION build)
   ...
   단면은 SimpleBEAM 과 같은 DB 를 쓰고 스타일도 같은 벌이다(.bf-* 규칙이 두
   파일에서 같다). 다만 <style> 의 id 는 달라야 한다: 같으면 먼저 뜬 쪽이 심어
   놓은 것을 보고 나중 쪽이 건너뛰어, 이 파일에만 있는 .cb-* 규칙이 통째로 빠진다.

   검사: tools/check_beam_multi.js — 36항목.

   진입점: fbeam_multi(mountId). layout_body.js 가 필요할 때 로드한다.

   테스트본(beam_multi_test.js)의 사본이다. 고치는 것은 언제나 테스트본이 먼저이고,
   확인이 끝나면 여기로 옮긴다 — 저장소의 다른 모듈과 같은 방식:
       cp beam_multi_test.js beam_multi.js
       # 머리주석과 엔진 ?v= 를 운영값으로 되돌리고,
...
(function () {
  ...
  function start(mountId) {
    ...
    sc.src = BASE + 'beam_engine.js?v=2';   // 운영은 고정 버전
