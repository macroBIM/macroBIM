/*
    bim_lwall.js — L-shaped (cantilever) retaining wall, parametric section (macroBIM Drawings)
    Same layout as the inverted-T wall but with NO toe: the base front is flush with the stem front.
    Entry point: fdraw_lwall(mountId)   [mountId default: 'mount-draw-lwall']
    Pure vanilla JS + inline SVG (no Konva / no external deps). Styles are scoped to .lw-root.
*/
(function () {
  "use strict";

  var _lwDraw = null, _lwRT = null;   // current instance's draw() + debounce timer
  window.addEventListener("resize", function () {
    clearTimeout(_lwRT);
    _lwRT = setTimeout(function () { if (_lwDraw) _lwDraw(); }, 120);
  });

  var CSS =
    ".lw-root{--dim:#2563eb;--slope:#1f8e9e;--soil:#b4813a;--found:#6e7e8c;--foundfill:#eef2f6;" +
    ...
    ".lw-mono{font-family:ui-monospace,'SF Mono',Menlo,Consolas,monospace;font-variant-numeric:tabular-nums}" +
    ".lw-grid{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(0,1fr);gap:20px;align-items:start}" +
    "@media(max-width:900px){.lw-grid{grid-template-columns:1fr}}" +
    ".lw-card{background:var(--panel);border:1px solid var(--line);border-radius:10px;overflow:hidden}" +
    ".lw-hd{display:flex;justify-content:space-between;align-items:center;padding:11px 16px;border-bottom:1px solid var(--hair);background:var(--chip)}" +
    ".lw-ttl{font-size:15px;font-weight:600;color:#0f172a;display:inline-flex;align-items:center}" +
    ".lw-ttl::before{content:'';display:inline-block;width:4px;height:15px;border-radius:2px;background:#2563eb;margin-right:9px;flex-shrink:0}" +
    ".lw-plot{display:block;width:100%;height:auto;background:" +
    ...
    ".lw-inputs{padding:14px}" +
    ".lw-inrow{display:grid;grid-template-columns:1fr auto;align-items:center;gap:8px;padding:5px 0;border-bottom:1px dashed var(--hair)}" +
    ".lw-inrow:last-child{border-bottom:0}" +
    ".lw-inrow label{font-size:13px;display:flex;align-items:baseline;gap:8px}" +
    ".lw-inrow .var{font-weight:600;color:var(--dim);min-width:34px;display:inline-block;font-family:ui-monospace,Menlo,Consolas,monospace}" +
    ".lw-inrow .desc{color:var(--muted);font-size:12px}" +
    ".lw-inrow input{width:96px;text-align:right;padding:5px 8px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--ink);font-size:13px;font-variant-numeric:tabular-nums}" +
    ".lw-inrow input:focus{outline:2px solid var(--dim);outline-offset:1px;border-color:var(--dim)}" +
    ".lw-unit{color:var(--muted);font-size:11px;margin-left:6px}" +
    ".lw-plot{cursor:grab;touch-action:none;-webkit-user-select:none;user-select:none}.lw-plot:active{cursor:grabbing}" +
    ".lw-hd-r{display:flex;align-items:center;gap:10px}" +
    ".lw-btn{font:inherit;font-size:10.5px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#fff;" +
    ...
    ".lw-btn:hover{filter:brightness(1.12)}.lw-btn:active{filter:brightness(.94);transform:translateY(1px)}" +
    ".lw-batch-wrap{padding:0 0 10px;margin-bottom:8px;border-bottom:1px dashed var(--hair)}" +
    ".lw-batch-lbl{font-size:10px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;color:var(--muted);margin-bottom:5px}" +
    ".lw-batch-hint{font-weight:400;text-transform:none;letter-spacing:0;color:var(--dim);font-family:ui-monospace,Menlo,Consolas,monospace;font-size:10px}" +
    ".lw-batch{width:100%;resize:none;font-family:ui-monospace,Menlo,Consolas,monospace;font-size:12px;line-height:1.5;padding:6px 8px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--ink)}" +
    ".lw-batch:focus{outline:2px solid var(--dim);outline-offset:1px;border-color:var(--dim)}";
  ...
  function buildMarkup() {
    return "" +
      ...
      "  <div class='lw-grid'>" +
      "    <div class='lw-card'>" +
      "      <div class='lw-hd'><span class='lw-ttl'>Layout</span>" +
      "        <span class='lw-hd-r'><button type='button' class='lw-btn' data-lw-fit>Reset view</button>" +
      "        <span class='lw-ttl lw-mono'>SCALE&nbsp;NTS</span></span></div>" +
      "      <svg class='lw-plot' viewBox='0 0 620 724' role='img' aria-label='L-shaped wall section (scroll to zoom, drag to pan)'></svg>" +
      "    </div>" +
      "    <div class='lw-card'>" +
      "      <div class='lw-hd'><span class='lw-ttl'>Dimension Input &mdash; live redraw on edit</span>" +
      "        <button type='button' class='lw-btn' data-lw-dxf>DXF out</button></div>" +
      "      <div class='lw-inputs'></div>" +
      "    </div>" +
      "  </div>" +
      "</div>";
  }
  ...
  function initGW(root) {
    ...
    // ---- input form ----
    var box = root.querySelector(".lw-inputs");
    ...
    bwrap.className = "lw-batch-wrap";
    bwrap.innerHTML =
      "<div class='lw-batch-lbl'>Batch Input (CSV) <span class='lw-batch-hint'>" + order.join(",") + "</span></div>" +
      "<textarea class='lw-batch' rows='2' spellcheck='false'></textarea>";
    box.appendChild(bwrap);
    var batchTa = bwrap.querySelector(".lw-batch");
    ...
    VARS.forEach(function (v) {
      ...
      row.className = "lw-inrow";
      row.innerHTML =
        "<label><span class='var'>" + k + "</span><span class='desc'>" + v[2] + "</span></label>" +
        "<span><input class='lw-mono' type='number' step='" + step + "' value='" + v[1] + "' data-k='" + k + "'>" +
        "<span class='lw-unit'>" + unit + "</span></span>";
      box.appendChild(row);
    });
    ...
    // ---- drawing ----
    var svg = root.querySelector(".lw-plot");
    ...
    var fitBtn = root.querySelector("[data-lw-fit]");
    ...
    function draw() {
      ...
      var Hs = P.Hs, st = P.st, fo = P.fo, bo = P.bo, tb = P.tb, heel = P.heel, hh = P.hh,
        tsb = P.tsb, hk = P.hk, bk = P.bk, kx = P.kx, tbl = P.tbl, ff = P.ff, fb = P.fb, N = P.N, bh = P.bh, H0 = P.H0, Df = P.Df;
      ...
      // L-shaped wall: NO toe — the base front is flush with the stem front (x = 0)
      var baseFront = 0, baseBack = sb + heel, B = sb + heel;
      var tsfc = 0;                                    // no toe → no toe-top slope
      var tsbc = Math.max(0, Math.min(tsb, tb * 0.8)); // heel-top slope drop (clamped)
      var bB = tb;                                     // slab thickness at the front = tb (no toe slope)
      ...
      // [structure] wall outline: stem + base slab (heel top slopes) + heel haunch; front flush at x=0
      var wall = [
        ...
        [0, -bB],                                      // base bottom to the front (no toe)
        [0, 0]                                         // base front face up to stem front base
      ];
      ...
      // front (passive-side) soil in front of the stem: surface height Df measured from the slab bottom (-bB)
      ...
      var fgL = baseFront - Math.max(900, sb);          // front-soil left extent (no toe to base it on)
      var frontSoil = [[xfDf, Dfy], [fgL, Dfy], [fgL, 0], [0, 0]];
      ...
      var availH = vpH;
      var cards = root.querySelectorAll(".lw-card");
      var secHd = svg.parentNode && svg.parentNode.querySelector(".lw-hd");
      ...
      var defs = el("defs", {});
      defs.innerHTML =
        "<pattern id='lwConc' width='7' height='7' patternTransform='rotate(45)' patternUnits='userSpaceOnUse'>" +
        "<line x1='0' y1='0' x2='0' y2='7' stroke='var(--concrete-ln)' stroke-width='0.7'/></pattern>" +
        "<pattern id='lwBlind' width='6' height='6' patternTransform='rotate(-45)' patternUnits='userSpaceOnUse'>" +
        "<rect width='6' height='6' fill='var(--foundfill)'/>" +
        "<line x1='0' y1='0' x2='0' y2='6' stroke='var(--found)' stroke-width='0.6'/></pattern>" +
        "<pattern id='lwSoil' width='14' height='10' patternUnits='userSpaceOnUse'>" +
      ...
      g.appendChild(el("polygon", { points: pts(soilPoly), fill: "url(#lwSoil)", stroke: "none" }));

      // --- front (passive) soil on the toe ---
      if (hasFront) g.appendChild(el("polygon", { points: pts(frontSoil), fill: "url(#lwSoil)", stroke: "none" }));

      // --- foundation: blinding concrete (distinct hatch + dashed outline) ---
      blinds.forEach(function (poly) {
        g.appendChild(el("polygon", { points: pts(poly), fill: "url(#lwBlind)", stroke: "var(--found)", "stroke-width": 1, "stroke-dasharray": "4 2" }));
      });

      // --- wall structure: stem + base + haunch (+ shear key), one monolithic concrete ---
      (hasKey ? [wall, key] : [wall]).forEach(function (poly) {
        g.appendChild(el("polygon", { points: pts(poly), fill: "url(#lwConc)", stroke: "var(--ink)", "stroke-width": 1.8, "stroke-linejoin": "round" }));
      });
      ...
      // ABOVE the base — stem base width sb, heel length (no toe on an L-wall)
      var Yabove = SY(0) - 46, Ylab = Yabove - 9;
      vwit(0, 0, Yabove); vwit(sb, 0, Yabove); vwit(baseBack, 0, Yabove);
      dimLine(g, SX(0), Yabove, SX(sb), Yabove);
      dimLine(g, SX(sb), Yabove, SX(baseBack), Yabove);
      txt(g, (SX(0) + SX(sb)) / 2, Ylab, "sb = " + sb, "d");                            // stem base width
      txt(g, (SX(sb) + SX(baseBack)) / 2, Ylab, "heel = " + heel, "d");                 // heel (wide)
      ...
      // (L-wall has no toe → no toe-top slope dimension)
      // front soil depth Df (front side): from the slab bottom up to the front ground surface; sits OUTER (swapped with tb)
      if (hasFront) {
        var dfCol = col2;
    ...
    function buildDXF() {
      ...
      var baseFront = 0, baseBack = sb + heel, B = sb + heel;   // L-wall: no toe, base front flush with stem front
      var tsfc = 0, tsbc = Math.max(0, Math.min(tsb, tb * 0.8));
      var bB = tb;                                     // slab thickness at the front = tb
      ...
      W(0, 0, 0, yA); W(sb, 0, sb, yA); W(baseBack, 0, baseBack, yA);
      DIMH(0, sb, yA, "sb = " + sb); DIMH(sb, baseBack, yA, "heel = " + heel);
    ...
    function downloadDXF() {
      ...
      a.href = url; a.download = "LWall.dxf";
      document.body.appendChild(a); a.click(); a.remove();
      setTimeout(function () { URL.revokeObjectURL(url); }, 1500);
    }
    var dxfBtn = root.querySelector("[data-lw-dxf]");
    if (dxfBtn) dxfBtn.addEventListener("click", downloadDXF);

    _lwDraw = draw;   // latest instance; a single module-level resize listener refits it
    draw();
  }

  // ---- public entry point (matches macroBIM fdraw_* convention) ----
  window.fdraw_lwall = function (mountId) {
    mountId = mountId || "mount-draw-lwall";
    var mount = document.getElementById(mountId);
    if (!mount) return;
    if (mount.querySelector(".lw-root")) return; // already built — keep current state
    mount.innerHTML = buildMarkup();
    initGW(mount.querySelector(".lw-root"));
  };
})();
