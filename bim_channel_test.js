/*
    bim_channel_test.js — TEST build of the Channel drawing (retaining-wall style).
    ...
    - The view is chosen from buttons in the card header via chan_setview().
    - Overrides fdraw_channel() from bim_channel.js and reuses its geometry (geo_channel)
      + DXF (odxf_channel) + 3D (render_channel_3d) helpers. Loaded on demand by
      layout_body_test.js only — production bim_channel.js is unaffected.
*/
(function () {
  "use strict";

  window._chanView = window._chanView || 'front';

  var INK = '#182430', DIM = '#2563eb', HID = '#94a3b8', CEN = '#e5484d';

  function setActive(view) {
    var bar = document.getElementById('chan-viewbar');
    if (!bar) return;
    Array.prototype.forEach.call(bar.querySelectorAll('[data-cview]'), function (b) {
  ...
  // ---- per-view geometry in model coords (origin = bottom-left web outside, y up) ----
  ...
  function buildView(view, geo, aparam, dseg_leng) {
    ...
    if (view === 'front' || view === 'back') {
      ...
      var pbti = gp('pbti'), pbft = gp('pbft'), ptft = gp('ptft');
      DL.push({ x1: pbw.x - off, y1: pbw.y, x2: pbw.x - off, y2: ptw.y, gap: ext * 6, t: 'H' });
      DL.push({ x1: pbw.x - off, y1: pbw.y, x2: pbw.x - off, y2: pbft.y, gap: ext * 3, t: 'tf' });
      DL.push({ x1: pbw.x - off, y1: ptft.y, x2: pbw.x - off, y2: ptw.y, gap: ext * 3, t: 'tf' });
      DL.push({ x1: ptw.x, y1: ptw.y + off, x2: ptf.x, y2: ptf.y + off, gap: ext * 6, t: 'B' });
      DL.push({ x1: ptw.x, y1: ptw.y + off, x2: pbti.x, y2: ptw.y + off, gap: ext * 3, t: 'tw' });
      ...
      var top = view === 'top';
      var oL = top ? ['ptw', 'ptf'] : ['pbw', 'pbf'];
      var o0 = gp(oL[0]), o1 = gp(oL[1]), pbti = gp('pbti');
      ...
      L.push({ x1: -half, y1: pbw2.y, x2: -half, y2: ptw2.y, lay: S });
      L.push({ x1: half, y1: pbw2.y, x2: half, y2: ptw2.y, lay: S });
      DL.push({ x1: -half - off, y1: pbw2.y, x2: -half - off, y2: ptw2.y, gap: ext * 6, t: 'H' });
      DL.push({ x1: -half - off, y1: pbw2.y, x2: -half - off, y2: pbft2.y, gap: ext * 3, t: 'tf' });
      DL.push({ x1: -half - off, y1: ptft2.y, x2: -half - off, y2: ptw2.y, gap: ext * 3, t: 'tf' });
      DL.push({ x1: -half, y1: ptw2.y + off, x2: half, y2: ptw2.y + off, gap: ext * 6, t: 'L' });
    }
  ...
  function render3d(dd) {
    function go() { if (typeof render_channel_3d === 'function') render_channel_3d('chan3d', dd.geo, dd.geo, dd.dseg_leng); }
    if (typeof render_channel_3d === 'function' && typeof THREE !== 'undefined') { go(); return; }
    var host = document.getElementById('chan3d');
    ...
    if (typeof render_channel_3d !== 'function') urls.push('https://macrobim.github.io/macroBIM/bim_channel_3d.js');
    (function next(i) {
  ...
  function renderView() {
    var host = document.getElementById(scvs_channel);
    if (!host || typeof _channel_drawData === 'undefined' || !_channel_drawData) return;
    var dd = _channel_drawData, view = window._chanView;
    host.innerHTML = '';
    setActive(view);
    var Hpx = matchHeight();
  ...
  window.fdraw_channel = function () {
    ...
    var u = getParams_channel();
    var aparam = u.aparam, dseg_leng = u.dseg_leng;
    var ta = document.getElementById('sUserText');
    if (ta) ta.value = u.combText + "\n" + dseg_leng;

    var geo = geo_channel(aparam);
    var _col = Math.max(aparam.db, dseg_leng) * 1.5, _row = aparam.dh * 2.0;

    geo.lines.forEach(function (l) { odxf_channel.line(l.x1, l.y1, l.x2, l.y2, alayer[0]); });
    geo.arcs.forEach(function (a) { odxf_channel.arc(a.x, a.y, a.r, a.angb, a.ange, alayer[0]); });
    ...
    function _dxf_long(ox, oy, names, hidden, axis) {
      ...
      names.forEach(function (n) {
        var p = _gp(n);
        if (axis === 'top') odxf_channel.line(ox + p.x, oy - half, ox + p.x, oy + half, alayer[0]);
        else odxf_channel.line(ox - half, oy + p.y, ox + half, oy + p.y, alayer[0]);
      });
      if (names.length >= 2) {
        var n1 = _gp(names[0]), n2 = _gp(names[names.length - 1]);
        if (axis === 'top') {
          odxf_channel.line(ox + n1.x, oy - half, ox + n2.x, oy - half, alayer[0]);
          odxf_channel.line(ox + n1.x, oy + half, ox + n2.x, oy + half, alayer[0]);
        } else {
          odxf_channel.line(ox - half, oy + n1.y, ox - half, oy + n2.y, alayer[0]);
          odxf_channel.line(ox + half, oy + n1.y, ox + half, oy + n2.y, alayer[0]);
        }
      }
      hidden.forEach(function (n) {
        var p = _gp(n);
        if (axis === 'top') odxf_channel.line(ox + p.x, oy - half, ox + p.x, oy + half, alayer[1]);
        else odxf_channel.line(ox - half, oy + p.y, ox + half, oy + p.y, alayer[1]);
      });
    }
    ...
    _channel_drawData = { geo: geo, aparam: aparam, dseg_leng: dseg_leng, alayer: alayer };
    renderView();
  };

  window.chan_setview = function (name) { window._chanView = name; renderView(); };

  // expose the pure renderers so the harness / other views can reuse them
  window._chanBuildView = buildView;
  window._chanRenderSVG = renderSVG;
})();
