...
(function () {
  ...
  function initGW(root) {
    ...
    function draw() {
      ...
      // front (passive-side) soil on the toe: surface at height Df above the base top, resting on the toe
      var Dfc = Math.max(0, Math.min(Df, Hs * 0.95));  // clamp under the stem top
      var hasFront = (Dfc > 0);
      var xfDf = frontOff * Dfc / Math.max(Hs, 1);     // stem front-face x at height Dfc
      var frontSoil = [[xfDf, Dfc], [baseFront, Dfc], [baseFront, -tsfc], [0, 0]];
      ...
      // --- front ground surface line + natural-ground hachure symbol (toe side) ---
      if (hasFront) {
        var fgY = SY(Dfc), fgXr = SX(xfDf), fgXl = SX(baseFront);
    ...
    function buildDXF() {
      ...
      var baseFront = -toe, baseBack = sb + heel, B = toe + sb + heel;
      var Dfc = Math.max(0, Math.min(Df, Hs * 0.95)), hasFront = (Dfc > 0), xfDf = frontOff * Dfc / Math.max(Hs, 1);
      ...
      if (hasFront) {                                                // front (passive) soil surface on the toe + hachures
        L(baseFront, Dfc, xfDf, Dfc, BROWN);
        for (var fi = 0; fi < 5; fi++) { var fhx = baseFront + asz * (1 + fi * 1.3); L(fhx, Dfc, fhx - asz * 0.7, Dfc - asz * 0.7, BROWN); }
      }
